...
class BlogPostViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        """
        # Always start with only published posts that have a published_at date
        queryset = BlogPost.objects.filter(
            status="published", published_at__isnull=False
        ).order_by("-published_at", "-created_at")

        # Remove the status filtering since we always want published only
        # category and featured filtering remains the same
    ...
    def featured(self, request):
        ...
        """
        featured_posts = BlogPost.objects.filter(
            status="published", published_at__isnull=False, is_featured=True
        ).order_by("-published_at")[:5]
