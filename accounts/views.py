...
class WishlistViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [IsAuthenticated]  # ADDED: Explicit authentication requirement

    def get_queryset(self):
        # FIXED: Only filter if user is authenticated (redundant with permission_classes, but safer)
        if self.request.user.is_authenticated:
            return WishlistItem.objects.filter(user=self.request.user)
        return WishlistItem.objects.none()  # Return empty queryset for anonymous users
